...
pull_mm_ndt<-function(start_date, end_date, query, state, state.abv){
  ...
  for(i in 1:length(day.starts)){
    ...
    #str_sub(date_1, 1,7)%>%str_split("-")%>%unlist%>%str_c(collapse="_")
    date_2<-date_1+1
    #date_2<-str_c(str_sub(date_1, 1,8), numberOfDays(ymd(parse_date_time(str_sub(date_1, 1,7), "%y-%m"))))
...
#    date_mm <- format(as.Date(day.starts), "%Y-%m")
#    date.mm.loc<-str_locate(query, "DATEMM")
#    date.mm.fir<-str_sub(query, 1,date.mm.loc[1]-1)
#    date.mm.last<-str_sub(query, date.mm.loc[2]+1, nchar(query))
#    query<-str_c(date.mm.fir, format(as.Date(day.starts), "%Y-%m"), date.mm.last)
